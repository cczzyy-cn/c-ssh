...
| 里程碑 | 内容 | 状态 |
|---|---|---|
| M0 脚手架 | Tauri + React + xterm.js 跑通（本地 echo 演示已随功能下线移除） | ✅ 已完成 |
...
| 项 | 说明 |
|---|---|
| 默认窗口 | 1100×700 → 1600×800（设置页可调，持久化） |
| 主题精简 | 删除内置 Solarized Dark、Gruvbox Dark（保留 6 套） |
| 按钮 hover | 修复所有主题 hover 色与按钮底色相同导致悬停无反馈 |
| Solarized Light | 按钮边框/分割线重新分配为暖色系（border `#d5cfbd`、divider `#e6e0cf`） |
| 其他 | 连接表单删除连接级主题选择；设置页字号调节不再因 label 关联触发聚焦 |
